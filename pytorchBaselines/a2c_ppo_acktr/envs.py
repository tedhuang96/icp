...
def make_vec_envs(env_name,
                  ...
                  ax=None, test_case=-1):
    if config is None:
        raise ValueError("config is None. Not able to make vec envs.")
    # envs = [
    ...
    # ]
    envs = [
        make_env(env_name, seed, i*int(config.env.test_size/num_processes), log_dir, allow_early_resets, config=config,
                 envNum=num_processes, ax=ax, test_case=test_case)
        for i in range(num_processes)
    ]
...
class VecPyTorch(VecEnvWrapper):
    ...
    def reset(self, phase='train', test_case=None, from_snapshot=False, use_orca=False):
        obs = self.venv.reset(phase, test_case, from_snapshot, use_orca)
