...
class Config(object):
    ...
    # * train_size, val_size, test_size are used for np random seed setup for conformal prediction robotics project.
    env.train_size = 1000000
    env.val_size = 0
    env.test_size = 8000 # number of episodes for test.py
    ...
    humans.policy = "orca"
    humans.radius = 0.4 # radius of each human
    humans.v_pref = 1 # max velocity of each human # 3-4mph, 3 mph 1.3411 m/s 4 mph 1.7882 m/s
    # FOV = this values * PI
    humans.FOV = 2.

    # a human may change its goal before it reaches its old goal. Related to problem difficulty. Add randomness of human motion.
    humans.random_goal_changing = True
    humans.random_goal_shift_max_times = 2
    humans.random_goal_shift_displacement_magnitude = 2. # 0., 0.5, 2

    # a human change its goal after it reaches its old goal, and assign a new human index
    ...
    # robot settings
    robot = BaseConfig()
    robot.visible = True # False  # the robot is visible to humans
    # robot policy: srnn for now
    robot.policy = 'orca' # 'srnn'
    robot.radius = 0.4#0.8#0.3  # radius of the robot
    robot.v_pref = 1#1  # max velocity of the robot
    ...
    gst = BaseConfig()
    # gst.obs_seq_len = 5 # in saved gst config for trained gst model, which is 5.
    # gst.invalid_value = -999.