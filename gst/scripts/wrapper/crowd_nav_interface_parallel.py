...
from gst.src.gumbel_social_transformer.st_model import st_model
...
def visualize_output_trajectory_deterministic(input_traj, input_binary_mask, output_traj, output_binary_mask, sample_index, obs_seq_len=5, pred_seq_len=5):
    ...
    full_obs_ped_idx = np.where((input_binary_mask_i.sum(1)[:,0]==obs_seq_len).to('cpu'))[0]
    full_traj = np.concatenate((input_traj_i.to('cpu'), output_traj_i[:,:,:2].to('cpu')), axis=1)
    output_binary_mask_i_pred_len = np.stack([output_binary_mask_i.to('cpu') for j in range(pred_seq_len)], axis=1)
    # import pdb; pdb.set_trace()
    loss_mask = np.concatenate((input_binary_mask_i.to('cpu'), output_binary_mask_i_pred_len), axis=1)
    fig, ax = plt.subplots()
...
if __name__ == '__main__':
    ...
    model = CrowdNavPredInterfaceMultiEnv(load_path=load_path,
                                          device=device, config = args_eval)
