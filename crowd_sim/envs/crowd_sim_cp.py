...
"""
Adapted from crowd_sim.py for random goal changing humans in conformal prediction project.
# ! Copied from crowd_sim_conformal_prediction.py
"""
...
class CrowdSim(gym.Env):
    def __init__(self):
        ...
        self.fig, self.render_axis = plt.subplots(figsize=(7, 7))
        self.render_axis.set_xlim(-8, 8)
        self.render_axis.set_ylim(-8, 8)
        ...
        self.snapshot_data = {}
        self.snapshot_data['robot_planned_trajectory'] = [] # initialize for reward calculation
        self.robot_snapshot_time_step_count = 0
        self.humans_index_current = []
    ...
    def configure(self, config):
        ...
        if self.config.humans.policy == 'orca' or self.config.humans.policy == 'social_force':
            ...
            self.case_capacity = {'train': self.config.env.train_size, 'val': self.config.env.val_size, 'test': self.config.env.test_size}
            self.case_size = {'train': self.config.env.train_size, 'val': self.config.env.val_size,
                              'test': self.config.env.test_size}
        ...
        self.last_human_goals = np.zeros((self.human_num, 2))
        ...
        self.obs_seq_len = config.gst.obs_seq_len

        self.random_goal_shift_max_times = config.humans.random_goal_shift_max_times
        self.random_goal_shift_displacement_magnitude = config.humans.random_goal_shift_displacement_magnitude
    ...
    def generate_random_human_position(self, human_num):
        ...
        for i in range(human_num):
            ...
            self.humans_index_current.append(i)
    ...
    def generate_circle_crossing_human(self):
        ...
        while True:
            ...
            # add some noise to simulate all the possible cases robot could meet with human
            v_pref = 1.0 if human.v_pref == 0 else human.v_pref
            px_noise = (np.random.random() - 0.5) * v_pref * 4
            py_noise = (np.random.random() - 0.5) * v_pref * 4
            px = self.circle_radius * np.cos(angle) + px_noise
            py = self.circle_radius * np.sin(angle) + py_noise
            collide = False
    ...
    def update_last_human_states(self, human_visibility, reset):
        """
        update the self.last_human_states array and self.last_human_goals array
        ...
        for i in range(self.human_num):
            self.last_human_goals[i, :] = np.array(self.humans[i].get_goal_position())
            if human_visibility[i]:
                humanS = np.array(self.humans[i].get_observable_state_list())
                # ! not handled for indexing but we do not care visibility now.
    ...
    def generate_robot_humans(self, phase, human_num=None):
        ...
        if self.group_human:
            ...
            while humans_left > 0:
                ...
                if humans_left <= 4:
                    if phase in ['train', 'val']:
                        ...
                        # ! don't use group_human, don't care about humans_index_current
                    else:
                        self.generate_random_human_position(human_num=humans_left)
                    humans_left = 0
                else:
            ...
            while True:
                ...
                px_r = np.cos(rand_angle + increment_angle) * (self.circle_radius-0.5)
                py_r = np.sin(rand_angle + increment_angle) * (self.circle_radius-0.5)
            ...
            while True:
                ...
                gx = np.cos(rand_angle + increment_angle) * (self.circle_radius-0.5)
                gy = np.sin(rand_angle + increment_angle) * (self.circle_radius-0.5)
        ...
        # for FoV environment
            ...
            # randomize starting position and goal position
                ...
                rand_angle = np.random.uniform(0, np.pi * 2)
                # rand_angle = 0
                px_r = np.cos(rand_angle) * self.circle_radius
                py_r = np.sin(rand_angle) * self.circle_radius
                increment_angle = np.pi # start at opposite side of the circle
                
                gx = np.cos(rand_angle + increment_angle) * self.circle_radius
                gy = np.sin(rand_angle + increment_angle) * self.circle_radius
    ...
    def generate_robot_humans_from_snapshot(self):
        
        robot_px, robot_py = self.snapshot_data['robot_position_current']
        robot_gx, robot_gy = self.snapshot_data['robot_goal']

        robot_px_next, robot_py_next = self.snapshot_data['robot_planned_trajectory'][1]
        robot_angle = np.arctan2(robot_py_next-robot_py, robot_px_next-robot_px)
        self.robot.set(robot_px, robot_py, robot_gx, robot_gy, 0, 0, robot_angle)
        # self.robot.set(px, py, gx, gy, 0, 0, np.pi/2)
        for i in range(len(self.snapshot_data['humans_position_current'])):
            human = Human(self.config, 'humans')
            human_px, human_py = self.snapshot_data['humans_position_current'][i]
            human_gx, human_gy = self.snapshot_data['humans_goal'][i]
            human_vx, human_vy = self.snapshot_data['humans_velocity_current'][i]
            human_angle = np.arctan2(human_vy, human_vx)
            human_v_pref = self.snapshot_data['humans_v_pref'][i]
            human.set(human_px, human_py, human_gx, human_gy, human_vx, human_vy, human_angle, v_pref=human_v_pref)
            self.humans.append(human)
            
        self.robot_snapshot_time_step_count = 0
        self.humans_index_current = self.snapshot_data['humans_index_current'] # [self.robot_snapshot_time_step_count]
        # print("crowd simulation reset: self.robot_snapshot_time_step_count = ", self.robot_snapshot_time_step_count)

    # reset function
    # def reset(self, phase='train', test_case=None):
    def reset(self, phase='train', test_case=None, from_snapshot=False, use_orca=False):
        # ! deprecated. using CrowdSimDict version now.
        """
        ...
        # * reset for goal changing behaviors in conformal prediction
        # self.human_num
        
        # random_goal_shift_max_times = 2
        nums_goal_shifts = np.random.randint(1, self.random_goal_shift_max_times+1, self.human_num)
    ...
    # Update the specified human's end goals in the environment randomly
    def update_human_goal_conformal_prediction(self, human):
        random_goal_shift_displacement = [-self.random_goal_shift_displacement_magnitude, self.random_goal_shift_displacement_magnitude] 
        # random_goal_shift_max_times = 2
        # Update human's goals randomly
        if not self.group_human:
            humans_copy = []
        ...
        while True:
            # gx = np.random.random()*random_goal_shift_displacement + human.gx
            # gy = np.random.random()*random_goal_shift_displacement + human.gy

            gx = np.random.random()*(random_goal_shift_displacement[1]-random_goal_shift_displacement[0])+random_goal_shift_displacement[0]+ human.gx
            gy = np.random.random()*(random_goal_shift_displacement[1]-random_goal_shift_displacement[0])+random_goal_shift_displacement[0] + human.gy
            # angle = np.random.random() * np.pi * 2
            # # add some noise to simulate all the possible cases robot could meet with human
            # v_pref = 1.0 if human.v_pref == 0 else human.v_pref
            # gx_noise = (np.random.random() - 0.5) * v_pref
    ...
    def calc_reward(self, action):
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            episode_info = "Collision" # Collision()
        elif reaching_goal:
            reward = self.success_reward
            done = True
            episode_info = "ReachGoal" # ReachGoal()
        elif self.robot_snapshot_time_step_count+1 == len(self.snapshot_data['robot_planned_trajectory']):
            reward = 0
            done = True
            episode_info = "Timeout-Snapshot" # Timeout() # * time out as not reaching the goal from snapshot, but still finished running trajectories in the snapshot .
            ...
            episode_info = "Danger" # Danger(dmin)
    ...
    def step(self, action):
        ...
        # self.check_goal_shift_threshold() # ! do not change goal. What happens?
    ...
    def check_goal_shift_threshold(self):
        for human_index, human in enumerate(self.humans):
            human_distance_to_goal = ((human.gx-human.px)**2+(human.gy-human.py)**2)**0.5
            if len(self.goal_shift_distance_thresholds[human_index]) > 0 and \
                human_distance_to_goal > self.goal_shift_distance_thresholds[human_index][0]:
                self.update_human_goal_conformal_prediction(human)
                self.goal_shift_distance_thresholds[human_index] = self.goal_shift_distance_thresholds[human_index][1:]

    def reset_human_goal_shift_distance_threshold(self, human_index):
        # random_goal_shift_max_times = 2
        n_goal_shift = np.random.randint(1, self.random_goal_shift_max_times+1)
        human = self.humans[human_index]
        human_distance_to_goal = ((human.gx-human.px)**2+(human.gy-human.py)**2)**0.5
        self.goal_shift_distance_thresholds[human_index] = np.sort(np.random.rand(n_goal_shift))*human_distance_to_goal
    ...
    def render(
        ...
        visualization_folder="tmp",
        robot_position_history=None,
    ):
        # traj_pred (num_peds, pred_seq_len, 2)
        # robot_plan (T, 2)
        # conformal_prediction_radius (pred_seq_len,)


        # self.render_axis.rcParams['animation.ffmpeg_path'] = '/usr/bin/ffmpeg'
        ...
        self.render_axis.add_artist(robot)
        artists.append(robot)

        # self.render_axis.legend([robot, goal], ['Robot', 'Goal'], fontsize=16)
        ...
        for arrow in arrows:
            self.render_axis.add_artist(arrow)
        ...
        if self.robot_fov < np.pi * 2:
            ...
            self.render_axis.add_artist(FOVLine1)
            self.render_axis.add_artist(FOVLine2)
            artists.append(FOVLine1)
            artists.append(FOVLine2)

        # add humans and change the color of them based on visibility
        human_circles = [plt.Circle(human.get_position(), human.radius, fill=False) for human in self.humans]
        human_goal_circles = [plt.Circle(human.get_goal_position(), human.radius/2, fill=True) for human in self.humans]


        for i in range(len(self.humans)):
            self.render_axis.add_artist(human_circles[i])
            ...
            self.render_axis.text(self.humans[i].px - 0.1, self.humans[i].py - 0.1, str(self.humans_index_current[i]), color='black', fontsize=12)

            self.render_axis.add_artist(human_goal_circles[i])
            artists.append(human_goal_circles[i])
            human_goal_circles[i].set_color(c='y')
            self.render_axis.text(self.humans[i].gx - 0.1, self.humans[i].gy - 0.1, str(self.humans_index_current[i]), color='black', fontsize=12)
        
        # plot predicted human positions
        if traj_pred is not None:
            n_humans, traj_pred_len, _ = traj_pred.shape
            assert n_humans == len(self.humans)
            # traj_pred (num_peds, pred_seq_len, 2)
            for i in range(len(self.humans)):
                # add future predicted positions of each human
                for j in range(traj_pred_len):
                    circle = plt.Circle(traj_pred[i, j],
                                        self.config.humans.radius, fill=False, color='tab:orange', linewidth=1.5)
                    
                    # circle = plt.Circle(traj_pred[i, j] + np.array([robotX, robotY]),
                    #                     self.config.humans.radius, fill=False, color='tab:orange', linewidth=1.5)
                    # circle = plt.Circle(np.array([robotX, robotY]),
                    #                     self.humans[i].radius, fill=False)
                    self.render_axis.add_artist(circle)
                    artists.append(circle)
        
            # plot conformal prediction radius
            if conformal_prediction_radius is not None:
                for i in range(len(self.humans)):
                    for j in range(traj_pred_len):
                        circle = plt.Circle(
                            traj_pred[i, j],
                            self.config.humans.radius+conformal_prediction_radius[j],
                            fill=False,
                            color='tab:blue',
                            linewidth=0.5,
                            linestyle='--')
        ...
        if robot_plan is not None:
            robot_plan = np.array(robot_plan)
            robot_plan_plot, = self.render_axis.plot(robot_plan[:, 0], robot_plan[:, 1], 'bo-', linewidth=1.5)
        
        if robot_position_history is not None:
            robot_position_history = np.array(robot_position_history)
            robot_position_history_plot, = self.render_axis.plot(robot_position_history[:, 0], robot_position_history[:, 1], 'ro-', linewidth=1.5)
        # fig_folder = "/home/zhe/conformal_prediction_robot_dev/tmp/"
        fig_folder = visualization_folder + "/"
        os.makedirs(fig_folder, exist_ok=True)
        if step is None:
            self.fig.savefig(fig_folder+str(self.global_time)+".png")
            # print(str(self.global_time)+".png saved.")
        else:
            self.fig.savefig(fig_folder+str(step)+".png")
            # print(str(step)+".png saved.")
        # self.render_axis.pause(0.1)
        for item in artists:
            item.remove() # there should be a better way to do this. For example,
            # initially use add_artist and draw_artist later on
        for t in self.render_axis.texts:
            t.set_visible(False)
        
        if robot_plan is not None:
            robot_plan_plot.remove()

        if robot_position_history is not None:
            robot_position_history_plot.remove()
